...
const deprecatedModTypes = ["text/m-objects", "text/montage-serialization"];
const whiteModTypes = ["text/mod-serialization", ...deprecatedModTypes];
                ...
                case "HTML":
                    if (appPackage.packageDescription.appcache) {
                        ...
                        if (file.relativeUrlBase) {
                            relativeLocation = relativeLocation.replace(file.relativeUrlBase, "");
                        }
                ...
                case "LINK":
                    if (config.cssEmbedding && element.getAttribute("rel") === "stylesheet") {
                        ...
                        if (href.indexOf(":") === -1) {
                            ...
                            href = element.getAttribute("href");
                            if (file.relativeUrlBase) {
                                relativeToFile = URL.relative(appPackage.buildLocation.replace(file.relativeUrlBase, ""), file.buildLocation);
                                href = href.replace(file.relativeUrlBase, "");
                            } else {    
                                relativeToFile = URL.relative(appPackage.buildLocation, file.buildLocation)
                                href = element.getAttribute("href");
                            }
                            
                            appPackageRelativePath = URL.resolve(
                                    href,
                                    relativeToFile
                                );
                            ...
                            if (cssText.length) {
                                styleElement.appendChild(file.document.createTextNode(cssText));
...
function rebaseScriptTag(element, file, config) {
    ...
    if (type === "application/javascript" || type === "text/javascript") {
        if (config.lint && file.package.isMainPackage()) {
            ...
            if (type === "text/javascript") {
                config.out.warn(
                    "Proper MIME type for JavaScript is application/javascript and should be omitted in " + file.path,
                );
    ...
    } else if (whiteModTypes.includes(type)) {
        if (deprecatedModTypes.includes(type)) {
            config.out.warn(`Deprecated ${type} block in ${file.path}. Use text/mod-serialization.`);
        }
        ...
        try {
            ...
            if (error instanceof SyntaxError) {
                config.out.warn("Syntax Error in Mod Serialization in " + file.path);
            } else {
                throw error;
            }
        }
