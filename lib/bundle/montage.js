...
var MontageBootstrap = require("mod/montage");
var URL = require("url2");
var Promise = require("bluebird");
var Require = require("mod/core/mr/require");
...
function bundleMontageHtml(element, file, config) {
    ...
    return loadMontageScript(element, file, config).then(function (loader) {
        var bootstrapBundle = collectMontageBootstrapBundle(loader, file.package, config);
        return Bundle.collectPreloadBundles(loader, file.package, config).then(function (preloadPhases) {
            ...
            if (file.relativeUrlBase) {
                const baseElement = file.document.createElement("base");
                baseElement.setAttribute("href", file.relativeUrlBase);
                element.parentNode.insertBefore(baseElement, element.parentNode.firstChild);
            }
        });
    });
}
...
function bundleMontageScript(bootstrapBundle, preloadFiles, element, file, config) {
    ...
    var plan = preloadFiles.map(function (phase) {
        return phase.map(function (shard) {
            ...
            var shardUrl = URL.relative(file.buildLocation, shard.buildLocation);
            if (file.relativeUrlBase) {
                shardUrl = shardUrl.replace(file.relativeUrlBase, "");
            }
            return shardUrl;
        });
    });
    ...
    var montagePackage = file.package.getPackage({ name: "mod" });
    ...
    if (file.relativeUrlBase) {
        toBundle = toBundle.replace(file.relativeUrlBase, "");
        toMontage = toMontage.replace(file.relativeUrlBase, "");
        toPromise = toPromise.replace(file.relativeUrlBase, "");
    }
...
function loadMontageScript(element, file, config) {
    ...
    var montageLocation = file.package.getPackage({ name: "mod" }).location;
    return MontageBootstrap.loadPackage(applicationLocation, config).then(function (applicationPackage) {
        return applicationPackage
            .loadPackage(montageLocation)
            ...
            .then(function () {
                return applicationPackage
                    .deepLoad(file.relativeLocation)
                    .catch(function (error) {
                        error.message =
                            "Can't find dependencies of HTML application " +
                    ...
                    .then(function () {
                        if (element.hasAttribute("data-module")) {
                            return applicationPackage
                                .deepLoad(element.getAttribute("data-module"))
                                .catch(function (error) {
                                    error.message =
                                        "Can't find dependencies of HTML application data-module " +
                                        JSON.stringify(element.getAttribute("data-module")) +
                                        " because " +
                                        error.message;
                                    throw error;
                                });
...
function collectMontageBootstrapBundle(loader, bundler, config) {
    var montagePackage = bundler.getPackage({ name: "mod" });
    ...
    loader.getPackage({ name: "mod" }).getModuleDescriptor("core/promise").bundled = true;
