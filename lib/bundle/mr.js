...
var MrBootstrap = require("mod/core/mr/bootstrap-node");
...
function bundleMrHtml(element, file, config) {
    ...
    return loadMrScript(element, file, config).then(function (loader) {
...
function loadMrScript(element, file, config) {
    ...
    return MrBootstrap.loadPackage(applicationLocation, config).then(
        function (loader) {
            return loader
                .loadPackage({ name: "mr" })
        ...
        function (error) {
            error.message = "Can't load package in " + JSON.stringify(file.path) + " because " + error.message;
            throw error;
        },
    );
}
