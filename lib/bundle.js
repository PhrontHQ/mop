...
function bundle(appPackage, config) {
    ...
    return (
        Promise.each(values(appPackage.files), function (file) {
...
function bundleHtml(file, config) {
    ...
    if (pkg.hasPackage({ name: "mod" })) {
        montagePackage = pkg.getPackage({ name: "mod" });
    ...
    visit(document, function (element) {
        if (element.nodeType === Node.ELEMENT_NODE && element.tagName === "SCRIPT" && element.hasAttribute("src")) {
            var src = element.getAttribute("src"),
                location = URL.resolve(file.buildLocation, (file.relativeUrlBase ? file.relativeUrlBase : "") + src);
            if (location === montageLocation) {
                jobs.push(bundleMontageHtml.bind(null, element, file, config));
            } else if (location === requireLocation) {
                jobs.push(bundleMrHtml.bind(null, element, file, config));
            }
        }
    });
    ...
    if (bundleConfig == null || typeof bundleConfig === "boolean") {
        ...
        throw new Error(
            'package.json "bundle" must be true or an array of bundle ' +
                "module identifiers. Got: " +
                bundler.packageDescription.bundle,
        );
    }
    ...
    return Promise.each(batches, function (batch) {
        return Promise.each(batch, function (ids) {
            return Promise.each(ids, function (id) {
                return loader.deepLoad(id).catch(function (error) {
                    error.message =
                        "Can't collect the dependencies of " +
                        JSON.stringify(id) +
                        " in package " +
                        loader.location +
                        " because " +
