...
function read(location, config) {
    ...
    return MontageBootstrap.loadPackage(location, config).then(function (appPackage) {
        return loadDeepPackages(appPackage, config)
            .then(function (pkg) {
                return verifyVersions(pkg, config);
            })
...
function verifyVersions(package, config) {
    return new Promise(function (resolve, reject) {
        if (package.hasPackage({ name: "mod" })) {
            var montage = package.getPackage({ name: "mod" });
            var montagePredicate = mopDescription.dependencies.mod;
        ...
        if (package.hasPackage({ name: "mr" })) {
            ...
            if (mrPredicate) {
                if (
                    ...
                    var mrErr =
                        "Mop only supports Montage Require (mr) " +
                        mrPredicate +
                        ".  This app has " +
...
function loadDeepPackages(package, config, memo) {
    ...
    if (!packageLockDependencies && config.packageLock.packages[""] && config.packageLock.packages[""].name == config.packageLock.name) {
        packageLockDependencies = config.packageLock.packages[""].dependencies;
    }
...
function readPackage(package, config, appPackage) {
    ...
    // option variations
    hashBuilder.update(
        [config.minify, config.appcache, config.shared]
            .map(function (option) {
                return option ? "1" : "0";
            })
            .join(""),
    ...
    var rootFilter = Glob.makeFilter(
        Location.toPath(appPackage.location),
        appPackage.config.packageDescription.exclude || [],
    );
    ...
    return config.fs
        .listTree(_path, function (name, _stat) {
            let packageJsonPath;
            ...
            if (name !== _path) {
                ...
                if (_stat.isDirectory()) {
                    ...
                    let filtered = filter(name, _stat);
                    if (filtered) {
                        packageJsonPath = path.join(name, "package.json");
                        return config.fs.isFile(packageJsonPath).then(function (isFile) {
                            if (isFile) {
                                return config.fs.read(packageJsonPath).then(function (content) {
                                    content = JSON.parse(content);
                                    return content.name && content.name.length > 0 ? false : filtered;
                                }).catch(function () {
                                    return false;
                                });
                            } else {
        ...
        .then(function (names) {
            ...
            return Promise.each(names, function (name) {
                ...
                return config.fs.read(name).then(function (content) {
...
function hashDependencies(package, memo) {
    ...
    if (!memo[package.location]) {
        ...
        var mappings = package.mappings;
        var dependencies = keys(mappings)
            .sort()
            .map(function (name) {
